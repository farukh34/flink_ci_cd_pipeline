...
insert into FlinkOut SELECT
  JSON_OBJECT ('event' VALUE JSON_OBJECT('host' VALUE `host`.`name`, 'uuid' VALUE `uuid`, 'created' VALUE `event`.`created`, 'code' VALUE `event`.`code`),
       ...
       'rule' VALUE JSON_OBJECT('id' VALUE '123456789', 'name' VALUE 'Notepad Making Network Connection', 'description' VALUE 'Detects suspicious network connection by Notepad'),
       'observables_list' VALUE ARRAY['NA'],
       'falsepositives' VALUE ARRAY['Unknown'] ,
       'tacticidlist' VALUE ARRAY['TA0011', 'TA0002', 'TA0005'],
       'techniqueidlist' VALUE ARRAY['T1055']
  ...
  )) FROM EdrTable WHERE ((REGEXP_EXTRACT(LOWER(cast(`process`.`executable` as VARCHAR)), '^.*\\notepad\.exe$') is not null AND `process`.`executable` is not null) )and not ((REGEXP_EXTRACT(cast(`destination`.`port` as VARCHAR), '^9100$') is not null AND `destination`.`port` is not null) );
