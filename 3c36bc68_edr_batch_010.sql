...
SET pipeline.name=3c36bc68_edr_batch010;
...
SELECT 
  JSON_OBJECT ('event' VALUE JSON_OBJECT('host' VALUE `host`.`name`, 'uuid' VALUE `uuid`, 'created' VALUE `event`.`created`, 'code' VALUE `event`.`code`),
       ...
       'rule' VALUE JSON_OBJECT('id' VALUE 'f0684847-f55d-4fdb-9f9c-7c3d105a14a6', 'name' VALUE 'Suspicious Environment Variable Has Been Registered_custom', 'description' VALUE 'Detects the creation of user-specific or system-wide environment variables via the registry. Which contains suspicious commands and strings'),
  ...
  )) FROM stream_edr WHERE (`organization`.`id`='3c36bc68') AND (( ( (REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^powershell$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^pwsh$') is not null)  )  OR  ( (REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^.*\\appdata\\local\\temp\\.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^.*c:\\users\\public\\.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^.*tvqqaamaaaaeaaaa.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^.*tvpqaaiaaaaeaa8a.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^.*tvqaaaeaaaaeabaa.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^.*tvoaaaaaaaaaaaaa.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^.*tvptaqeaaaaeaaaa.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^.*sw52b2tll.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^.*ludm9rzs.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^.*jbnzva2ut.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^.*sqbuahyabwbragualq.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^.*kabgb2ag8aawblac0a.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^.*jag4adgbvagsazqata.*$') is not null)  )  OR  ( (REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^suvy.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^sqbfaf.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^sqbuah.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^cwbha.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^awv4.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^aqbla.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^r2v0.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^dmfy.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^dgbha.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^dxnpbm.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^h4sia.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^y21k.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^cabhah.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^qzpc.*$') is not null OR REGEXP_EXTRACT(LOWER(cast(`Details` as VARCHAR)), '^yzpc.*$') is not null)  ) ) AND ((REGEXP_EXTRACT(LOWER(cast(`TargetObject` as VARCHAR)), '^.*\\environment\\.*$') is not null) ) )  
SELECT 
  JSON_OBJECT ('event' VALUE JSON_OBJECT('host' VALUE `host`.`name`, 'uuid' VALUE `uuid`, 'created' VALUE `event`.`created`, 'code' VALUE `event`.`code`),
       'observables' VALUE JSON_OBJECT('event' VALUE JSON_OBJECT ('code' VALUE  `event`.`code`, 'type' VALUE  `event`.`type`, 'category' VALUE  `event`.`category`, 'action' VALUE  `event`.`action`, 'channel' VALUE  `event`.`channel`),'host' VALUE JSON_OBJECT ('name' VALUE  `host`.`name`),'user' VALUE JSON_OBJECT ('name' VALUE  `user`.`name`, 'domain' VALUE  `user`.`domain`),'process' VALUE JSON_OBJECT ('name' VALUE  `process`.`name`, 'parent' VALUE  JSON_OBJECT ('name' VALUE  `process`.`parent`.`name`, 'executable' VALUE  `process`.`parent`.`executable`, 'command_line' VALUE  `process`.`parent`.`command_line`), 'executable' VALUE  `process`.`executable`, 'command_line' VALUE  `process`.`command_line`, 'working_directory' VALUE  `process`.`working_directory`, 'pe' VALUE  JSON_OBJECT ('original_file_name' VALUE  `process`.`pe`.`original_file_name`, 'description' VALUE  `process`.`pe`.`description`, 'company' VALUE  `process`.`pe`.`company`, 'product' VALUE  `process`.`pe`.`product`), 'hash' VALUE  JSON_OBJECT ('sha256' VALUE  `process`.`hash`.`sha256`, 'imphash' VALUE  `process`.`hash`.`imphash`)),'file' VALUE JSON_OBJECT ('name' VALUE  `file`.`name`, 'path' VALUE  `file`.`path`, 'hash' VALUE  JSON_OBJECT ('sha256' VALUE  `file`.`hash`.`sha256`, 'imphash' VALUE  `file`.`hash`.`imphash`)),'registry' VALUE JSON_OBJECT ('path' VALUE  `registry`.`path`, 'value' VALUE  `registry`.`value`),'dns' VALUE JSON_OBJECT ('question' VALUE  JSON_OBJECT ('name' VALUE  `dns`.`question`.`name`)),'source' VALUE JSON_OBJECT ('domain' VALUE  `source`.`domain`, 'ip' VALUE  `source`.`ip`, 'port' VALUE  `source`.`port`),'destination' VALUE JSON_OBJECT ('domain' VALUE  `destination`.`domain`, 'ip' VALUE  `destination`.`ip`, 'port' VALUE  `destination`.`port`),'winlog' VALUE JSON_OBJECT ('targetimage' VALUE  `winlog`.`targetimage`, 'grantedaccess' VALUE  `winlog`.`grantedaccess`, 'eventtype' VALUE  `winlog`.`eventtype`, 'initiated' VALUE  `winlog`.`initiated`, 'user' VALUE  `winlog`.`user`, 'state' VALUE  `winlog`.`state`, 'integritylevel' VALUE  `winlog`.`integritylevel`, 'eventnamespace' VALUE  `winlog`.`eventnamespace`, 'name' VALUE  `winlog`.`name`, 'operation' VALUE  `winlog`.`operation`, 'consumer' VALUE  `winlog`.`consumer`, 'filter' VALUE  `winlog`.`filter`)),
       'observer' value JSON_OBJECT ('type' VALUE `observer`.`type`),
       ...
       'uuid' VALUE UUID(),
       'alert' VALUE JSON_OBJECT('created' VALUE now(), 'kind' VALUE 'alert', 'level' VALUE 'high'),
       'rule' VALUE JSON_OBJECT('id' VALUE '1d7af5c9-2d8d-42f6-976f-4745cdad8c41', 'name' VALUE 'Suspicious Environment Variable Has Been Registered_custom_clone', 'description' VALUE 'Detects the creation of user-specific or system-wide environment variables via the registry. Which contains suspicious commands and strings'),
       'observables_list' VALUE ARRAY['NA'],
       'falsepositives' VALUE ARRAY['Unknown'] ,
       'tacticidlist' VALUE ARRAY['TA0005', 'TA0003'],
       'techniqueidlist' VALUE ARRAY['NA']
